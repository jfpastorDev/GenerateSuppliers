...
@Component
public class ExportCSVFile implements IExport<PrintRecord> {
	Logger logger = LoggerFactory.getLogger(ExportCSVFile.class);
	
	ExportType EXPORT_TYPE = ExportType.CSV;
	...
	public void export(List<? extends PrintRecord> list, String... args) throws Exception {
		if (args==null || args.length<2) {
			throw new Exception("Parametros incorrectos para exportar a fichero csv");
		}
		...
		Path completePath = Paths.get(args[0]+ FileSystems.getDefault().getSeparator() + args[1] + ".csv"); // file name
		...
		try {
			...
			logger.error("**** Se ha producido un error al generar el fichero csv: {} ****", e.getMessage());
			throw e;
		}
